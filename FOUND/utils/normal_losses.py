...
class NormalLoss(nn.Module):
	...
	def calc_loss(self, render_norm: torch.FloatTensor, target_norm: torch.FloatTensor, mask: torch.FloatTensor=None):
		raise NotImplementedError
	def forward(self, render_norm: torch.FloatTensor, target_norm: torch.FloatTensor, mask: torch.FloatTensor=None):
		...
		"""
		self.calc_loss(render_norm, target_norm, mask)

class L1(NormalLoss):
	"""L1"""
	def calc_loss(self, render_norm, target_norm, mask=None):
...
class NLL(NormalLoss):
	...
	def calc_loss(self, render_norm, target_norm, target_kappa, mask=None):
		...
		if mask is not None:
			...
			valid_mask = valid_mask > 0.0
			dot = dot[valid_mask]
			target_kappa = target_kappa[:, :, :, 0][valid_mask]
		else:
			target_kappa = target_kappa[:, :, :, 0]
