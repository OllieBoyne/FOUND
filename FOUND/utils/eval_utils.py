...
def err_to_colour(
    ...
    ndim = err.ndim
    colmin = torch.tensor(colmin)[(None,) * ndim].to(
        err.device
    )  # expand colmin to [..., 3]
...
def eval_exp(exp_dir, render=True):
    ...
    if settings.get("model", "FIND") == "FIND":
        ...
        FIND_cutoff_surface = np.load(
            os.path.join(settings["find_pth"], "templ_masked_faces.npy")
        )
        FIND_sole_faces = np.load(
            os.path.join(settings["find_pth"], "templ_sole_faces.npy")
        )
        FIND_sole_verts = np.unique(
            np.ravel(pred_mesh_trimesh.faces[FIND_sole_faces])
        )  # all vertices considered part of the sole

        sole_vert_positions = pred_mesh_trimesh.vertices[
            FIND_sole_verts
        ]  # save sole vertex positions to refind them after mesh pre-processing

        pred_mesh_trimesh.update_faces(
            ~np.isin(np.arange(pred_mesh_trimesh.faces.shape[0]), FIND_cutoff_surface)
        )
        ...
        FIND_sole_vert_idxs = np.argwhere(
            np.all(
                pred_mesh_trimesh.vertices[:, None, :]
                == sole_vert_positions[None, ...],
                axis=-1,
            )
        )[:, 0]

        FIND_sole_vertex_mask = np.isin(
            np.arange(pred_mesh_trimesh.vertices.shape[0]), FIND_sole_vert_idxs
        )  # mask of which vertices correspond to the sole
        FIND_sole_faces_mask = np.any(
            FIND_sole_vertex_mask[pred_mesh_trimesh.faces], axis=-1
        )  # mask of which faces are in sole

    else:
        pred_mesh_trimesh = cutoff_slice_FIND(pred_mesh_trimesh)

    # Convert to PyTorch3D
    p3d_from_trimesh = lambda mesh: Meshes(
        verts=torch.from_numpy(np.asarray(mesh.vertices)[None, ...]).float(),
        faces=torch.from_numpy(np.asarray(mesh.faces)[None, ...]),
    ).to(device)
    ...
    pred_sample_dict = modified_sample(
        pred_mesh, num_samples=10_000, return_normals=True
    )

    # Calculate errors for reporting - by considering samples over the surface
    errs = modified_chamf(
        pred_sample_dict["verts"],
        gt_sample_dict["verts"],
        x_normals=pred_sample_dict["normals"],
        y_normals=gt_sample_dict["normals"],
    )

    # Calculate errors for visualisation - by considering every vertex
    vis_errs = modified_chamf(
        pred_mesh.verts_padded(),
        gt_mesh.verts_padded(),
        x_normals=pred_mesh.verts_normals_padded(),
        y_normals=gt_mesh.verts_normals_padded(),
    )
    ...
    if settings.get("model", "FIND") == "FIND":
        ...
        sampled_vertex_mask = FIND_sole_faces_mask[
            pred_sample_dict["face_idxs"].cpu().detach().numpy()[0]
        ]
    ...
    vis_params = {
        "cham": dict(
            ...
            mag=1_000,
            units="mm",
            cutoffs=np.array([5, 10, 15, 20, 25]) * 1e-3,
            xscale="log",
        ),
        "cham_norm": dict(
            vmin=0,
    ...
    with Reporter(os.path.join(eval_dir, "report.txt")) as report:
        ...
        for L in ["cham", "cham_norm"]:
            ...
            table.field_names = ["Desc", "Mean", "Median", "RMSE"] + [
                f"% < {round(x*mag)}" for x in cutoffs
            ]
            ...
            for desc, x in zip(["pred2real", "real2pred"], ["x", "y"]):
                ...
                results[f"{L}_{desc}"] = {
                    **{k: metrics[k] * mag for k in ["mean", "median", "rmse"]},
                    **{
                        f"% < {round(c*mag)}": i * 100
                        for c, i in zip(cutoffs, metrics["cutoffs"])
                    },
                }
    ...
    if render:
        renderer: Renderer = Renderer(
            image_size=256, max_faces_per_bin=100_000, device=device
        )
        ...
        for i, (mesh, err_key) in enumerate(
            zip(
                [gt_mesh, pred_mesh, gt_mesh, pred_mesh],
            ...
            colours = err_to_colour(
                this_error,
                vmin=vis_params[vis_type]["vmin"],
                vmax=vis_params[vis_type]["vmax"],
            )
            mesh.textures = TexturesVertex(colours)

            res = renderer(
                mesh, R, T, render_normals=False, render_sil=False
            )  # render mesh
        ...
        cv2.imwrite(
            os.path.join(eval_dir, "eval.png"), cv2.cvtColor(out, cv2.COLOR_BGR2RGB)
        )
...
def eval_exps(exps_dir, render=True, gpus=0, set_context=True):
    ...
    expts = sorted(
        [
            os.path.join(exps_dir, f)
            for f in sorted(os.listdir(exps_dir))
            if os.path.isdir(os.path.join(exps_dir, f))
        ]
    )
    ...
    if isinstance(gpus, int):
        ...
        for thread in threads:
            thread.start()
        for thread in threads:
            thread.join()  # hold here until finished
    ...
    for metric in ["mean", "median", "rmse"]:
        ...
        items = [
            f"{i}_{j}"
            for i in ["cham", "cham_norm"]
            for j in ["pred2real", "real2pred"]
        ]
        for exp_name, exp_res in res.items():
            exps_table.add_row(
                [exp_name] + [f"{exp_res[i][metric]:.2f}" for i in items]
            )

        exps_table.add_row(
            ["AVG"]
            + [
                f"{np.mean([exp_res[i][metric] for exp_res in res.values()]):.2f}"
                for i in items
            ]
        )
