...
def calc_losses(res: dict, batch: dict, loss_list: list, aux: dict) -> dict:
    ...
    if "norm_al" in loss_list:
        ...
        loss_dict["norm_al"] = normal_crit_al(
            res["norm_xyz"], batch["norm_xyz"], mask=norm_mask
        )

    if "norm_nll" in loss_list:
        norm_mask = (res["sil"] > 0).unsqueeze(-1)
        loss_dict["norm_nll"] = normal_crit_nll(
            res["norm_xyz"],
            batch["norm_xyz"],
    ...
    if "smooth" in loss_list:
        loss_dict["smooth"] = mesh_laplacian_smoothing(
            res["new_mesh"], method="uniform"
        )
