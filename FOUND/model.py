...
class FIND(nn.Module):
    def __init__(self, find_dir: str, opt_posevec: bool = True, kp_labels: list = None):
        ...
        self.rot = nn.Parameter(
            torch.tensor([0.0, 0.0, 0.0], requires_grad=True).unsqueeze(0)
        )
        self.trans = nn.Parameter(
            torch.tensor([0.0, 0.0, 0.0], requires_grad=True).unsqueeze(0)
        )
        self.scale = nn.Parameter(
            torch.tensor([1.0, 1.0, 1.0], requires_grad=True).unsqueeze(0)
        )

        vec_size = 100  # needs customisation
        self.shapevec = nn.Parameter(
            torch.tensor([0.0] * vec_size, requires_grad=True).unsqueeze(0)
        )
    ...
    def forward(self):
        ...
        meshes = self.model.get_meshes(
            shapevec=self.shapevec, reg=reg, texvec=self.texvec, posevec=self.posevec
        )["meshes"]
        ...
        for name, param in self.named_parameters():
            for k in ["reg", "deform"]:
                if any(param is x for x in self.params[k]):
                    # only works with 1D
                    out[k][name] = [
                        f"{x:.4f}" for x in np.ravel(param.cpu().detach().numpy())
                    ]
